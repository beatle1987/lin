...
{
	unsigned int *addr; //added JH
	int base_addr; //added JH
	unsigned short * fpga_addr, reg; //added JH
	int ret = -ENODEV;
...
#elif defined(CONFIG_ISA) //added JH
	dev->base_addr=0xf3800300;
	dev->irq=1;
	base_addr = dev->base_addr;
	fpga_addr = ioremap(0x10400000, 0x700);
	if (!fpga_addr)
	  {
	    printk("Can't get memory for FPGA\n");
	    return -ENOMEM;
	  }

	reg =  readw(fpga_addr + (0x104/2));
	reg &= ~0x20;
	writew(reg , fpga_addr + (0x104/2)); //polarity reg set bit 5 to 0
	reg =  readw(fpga_addr + (0x104/2));

	reg =  readw(fpga_addr + (0x106/2));
	reg &= ~0x20;
	writew(reg , fpga_addr + (0x106/2)); //edge reg set bit 5 to 0
	reg =  readw(fpga_addr + (0x106/2));

	reg =  readw(fpga_addr + (0x102/2));
	reg |= 0x20;
	writew(reg , fpga_addr + (0x102/2)); //enable reg 1 bit 5
	reg =  readw(fpga_addr + (0x102/2));

	reg =  readw(fpga_addr + (0x100/2));
	reg |= 0x20;
	writew(reg , fpga_addr + (0x100/2)); //status reg 1 bit 5
	reg =  readw(fpga_addr + (0x100/2));
	...
	addr = ioremap(0xf3800300, 10 * 16);
	if (!addr)
	  return -ENOMEM;
	ret=smc_probe(dev, base_addr);
	iounmap(addr);
/*clear interrupt from the test */
	reg =  readw(fpga_addr + (0x100/2));
	reg |= 0x20;
	writew(reg , fpga_addr + (0x100/2)); //status reg 1 bit 5
	reg =  readw(fpga_addr + (0x100/2));
...
static int __init smc_probe_chip(struct net_device *dev, int ioaddr)
{
	...
	if ((ioaddr & 0x0000FFFF) != ((temp >> 3) & 0x3E0)) {
...
static void smc_interrupt(int irq, void * dev_id, struct pt_regs * regs)
{
	...
	/*clear interrupts in the FPGA */
	addr = (unsigned short *)0xf2400000;
	reg =  readw(addr + (0x100/2));
	reg |= 0x20;
	writew(reg , addr + (0x100/2)); //status reg 1 bit 5
	reg =  readw(addr + (0x100/2));
	GEDR |= GPIO_GPIO1;
